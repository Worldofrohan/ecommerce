...
function displayProducts(products){
  ...
  `<div class="product">
    <div class="img"> <img src="${product.img}"
...
categoryList.addEventListener("click",(e) => {
  ...
  selectedCategory==="All"?displayProducts(data):displayProducts(data.filter((product)=> product.category
  == selectedCategory));
})
}
...
txtSearch.addEventListener("keyup",(e)=>{
  ...
  if(value){
    displayProducts(data.filter((product)=> product.name.toLowerCase()
    .indexOf(value)!== -1));
  }
    else{
